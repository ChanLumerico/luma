...
class ReduceLROnPlateau(Scheduler):
    def __init__(
        ...
        if mode not in {"min", "max"}:
            raise UnsupportedParameterError(mode)
        if threshold_mode not in {"rel", "abs"}:
            raise UnsupportedParameterError(threshold_mode)

        self.mode_worse = np.inf if self.mode == "min" else -np.inf

    @property
    def new_learning_rate(self) -> float:
        if self.iter != self.last_epoch:
            self.last_epoch = self.iter
            current = self.valid_loss_arr[-1] if len(self.valid_loss_arr) > 0 else None

            if current is None:
                return self.lr_trace[-1] if len(self.lr_trace) > 0 else self.init_lr
            ...
            if self.num_bad_epochs > self.patience:
                new_lr_ = max(self.lr_trace[-1] * self.factor, self.min_lr)
                self.cooldown_counter = self.cooldown
                self.num_bad_epochs = 0
            else:
                new_lr_ = (
                    self.lr_trace[-1] if len(self.lr_trace) > 0 else self.init_lr,
                )
            

        new_lr = self.lr_trace[-1] if len(self.lr_trace) > 0 else self.init_lr
