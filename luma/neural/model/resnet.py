...
class _SE_ResNet(Estimator, Supervised, NeuralModel):
    ...
    def build_model(self) -> None:
        ...
        self.model.extend(
            AdaptiveAvgPool2D((1, 1)),
            Flatten(),
            Dense(512 * Bottleneck_SE.expansion, self.out_features, **base_args),
        )
    ...
    @Tensor.force_shape(input_shape)
    def fit(self, X: Tensor, y: Matrix) -> Self:
        return super(_SE_ResNet, self).fit_nn(X, y)

    @override
    @Tensor.force_shape(input_shape)
    def predict(self, X: Tensor, argmax: bool = True) -> Matrix | Vector:
        return super(_SE_ResNet, self).predict_nn(X, argmax)
    ...
    def score(
        ...
        return super(_SE_ResNet, self).score_nn(X, y, metric, argmax)
